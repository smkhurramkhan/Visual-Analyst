...
class Camera2BasicFragment : Fragment(){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        button = view.findViewById<View>(R.id.detectnow) as Button
        button!!.setOnClickListener {
            imageUri = getImageUri(requireContext(), img)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE) {
            ...
            if (resultCode == -1) {
                Utility.uri = imageUri
                val intent = Intent(context, UploadImage::class.java)
    ...
    private fun classifyFrame() {
        ...
        val textToShow = bitmap?.let { classifier!!.classifyFrame(it) }
        bitmap!!.recycle()
        img =
            textureView!!.getBitmap(ImageClassifier.DIM_IMG_SIZE_X, ImageClassifier.DIM_IMG_SIZE_Y)
        Utility.bitmap = img

//    Utility.bitmap =   textureView.getBitmap(ImageClassifier.DIM_IMG_SIZE_X, ImageClassifier.DIM_IMG_SIZE_Y);
        if (textToShow != null) {
